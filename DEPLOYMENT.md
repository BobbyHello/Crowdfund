...
This guide walks through deploying Folio from a fresh clone to a working live demo. Two pieces deploy independently:
...
1. Resolves the native XLM Stellar Asset Contract id for testnet.
2. Builds both crates (`main` + `receipt`) to `wasm32v1-none/release`.
3. Deploys the receipt token with the deployer as placeholder admin.
4. Deploys the main contract with `--receipt <id>` and `--token <XLM SAC id>`.
5. Transfers the receipt admin role to the main contract.
6. Writes `NEXT_PUBLIC_MAIN_CONTRACT_ID` and `NEXT_PUBLIC_TOKEN_CONTRACT_ID` into `.env.local`.

### Custom token

Supporter Badge - SEP-41 soulbound badge, minted on each pledge. The deploy script transfers the token's admin role to the main contract during the run, so post-deploy only the main contract can mint.
